...
- `get_document_content` はAPIの `x-quota-compute-remaining` 差分から 2 / 4 / 8 credits の実消費を推定できる場合は実値で会計し、取得できない場合は最大8 creditsとして保守的に会計します。
- `get_document_content` はtool call全体で最大 `MOMONGA_MCP_MAX_CHARACTERS_PER_CONTENT_CALL` 文字を返します。続きは `content_sections[].next_offset` と同じ単一 `section_id` で取得します。
...
| `MOMONGA_MCP_MAX_LIST_LIMIT` | いいえ | `20` | list系toolの実行時最大件数です。API schema maximumは50です。 |
| `MOMONGA_MCP_MAX_SEARCH_TOP_K` | いいえ | `10` | search系toolの実行時最大件数です。API schema maximumは50です。 |
| `MOMONGA_MCP_MAX_SECTIONS_PER_CONTENT_CALL` | いいえ | `3` | `get_document_content` 1回あたりの最大section数です。API schema maximumは5です。 |
| `MOMONGA_MCP_MAX_CHARACTERS_PER_CONTENT_CALL` | いいえ | `30000` | `get_document_content` 1回あたりの最大本文文字数です。 |
