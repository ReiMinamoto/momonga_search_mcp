...
2. List candidate documents.
   ...
   - Default runtime result limit is 20 (overridable via `MOMONGA_MCP_MAX_LIST_LIMIT`); API schema maximum is 50.
...
3. Search within documents.
   ...
   - Default runtime `top_k` limit is 10 (overridable via `MOMONGA_MCP_MAX_SEARCH_TOP_K`); API schema maximum is 50.
   - Results are returned with `matches[]` ordered by `score` (highest first). Pick the top 1–3 matches whose `section_title` / `snippet` clearly align with the question; do not blindly pass every `section_id` to Step 6 (the content section limit is 3 by default, 5 max).
