...
| `get_document_content` | `GET /v1/documents/{document_id}/content` | 2/4/8/API call | yes | API実消費は本文量で変わりますが、MCP側は現状、最大8 creditsとして保守的に会計します。 |
...
| `next_cursor` | 続きの一覧取得に必要。 |
| `credits_used` | この tool call で消費した credit。 |
| `session_credits_used` | MCP session 内の累計 credit 使用量。 |
| `session_credit_limit` | MCP session の credit 上限。 |
| `session_credits_remaining` | MCP session 内で残っている credit。 |
