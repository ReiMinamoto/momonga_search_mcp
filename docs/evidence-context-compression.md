...
| `search_section_contents` | 短い excerpt を返す | 固定 match 上限と `context_chars` で制限する |
...
- section inline: `character_count` が inline threshold 以下。
- section manifest: inline threshold を超える section。
- call section limit: `get_document_content` は1回で扱うsection数を制限する。
- window: default と hard max を分けて持つ。
- search excerpt: `context_chars` と固定 match 上限を持つ。
...
通常は巨大 document を丸ごと取得してから切るのではなく、`get_document_toc`、section selection、`get_document_content(section_ids=...)` の順に誘導する。`section_ids` を省略した full document retrieval は、ユーザーが全文を1つの synthetic section としてcacheする必要を明示し、tool call で `allow_full_document=true` を渡した場合だけ使う。その場合でも inline 閾値を超える本文は manifest になり、本文確認は search/window retrieval に進める。

少なくとも、section inline threshold、section count limit、TOC direct threshold、section window default / hard max、search excerpt limits、evidence excerpt max は分けて持つ。
